...
2. **For each file in `drop/`:**
   ...
   c. **Move the file** to `knowledge/{area}/`
   d. **Add an entry** to `memory/_index.md` under the matching area section (path + 1-sentence description)
   e. **Add an entry** under "Recently Added" at the bottom
